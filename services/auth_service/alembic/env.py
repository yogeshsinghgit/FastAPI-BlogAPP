...
from db.database import Base  # Import your SQLAlchemy models

# Alembic Config object, which provides access to values in .ini file
config = context.config
settings = get_settings()

print(f"\n\nAlembic is using database URL: {settings.DATABASE_URL}\n\n")


config.set_main_option("sqlalchemy.url", settings.DATABASE_URL)  # Set DB URL
...
async def run_migrations_online():
    ...
    async with engine.begin() as conn:
        await conn.run_sync(do_run_migrations)  # ✅ Pass a function, not a lambda
